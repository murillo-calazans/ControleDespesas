...
Se tipo="despesa" e a mensagem indicar uma compra parcelada — números como "2/5", "parcela 2 de 5", "em 5x", "3x de 100" — preencha "parcela_atual" e "parcela_total" com inteiros (ex.: "2/5" -> parcela_atual=2, parcela_total=5). Duas formas de indicar isso, com regras DIFERENTES pro cálculo de "valor" — preste atenção em qual delas a mensagem usa:
1. A mensagem cita o número de QUAL parcela é, no formato "N/M" ou "parcela N de M" (ex.: "2/5", "6/7", "12/12", "parcela 2 de 5") — o valor informado JÁ é o valor dessa UMA parcela, nunca divida, use direto. Isso vale mesmo sem a palavra "parcela" explícita: "Roupas R$33,78 nubank 2/5" -> valor=33.78 (não 33.78/5). "R$65,74 1/4" -> valor=65.74 (não 65.74/4).
2. A mensagem descreve o parcelamento como "em Nx" ou "N vezes" SEM citar qual parcela é (ex.: "sofá de 3000 em 6x", "3x de 100") — nesse caso o valor dado é o TOTAL da compra: calcule "valor" como total/N (3000/6=500) pra achar o valor de UMA parcela, e assuma parcela_atual=1. Exceção: se já vier "N vezes de <valor>" (ex.: "3x de 100"), esse <valor> já é o de uma parcela — não divida de novo.
Se não houver menção a parcelamento, "parcela_atual" e "parcela_total" ficam null. Uma compra parcelada nunca é "fixo" — se detectar parcelamento, marque "fixo"=false.
